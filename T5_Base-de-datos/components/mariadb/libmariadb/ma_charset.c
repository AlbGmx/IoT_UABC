...
static unsigned int check_mb_utf8_sequence(const char *start, const char *end) {
   ...
   if (c < 0xF5) {
      ...
      */
      ...
      if (!(((uchar)start[1] ^ 0x80) < 0x40 && ((uchar)start[2] ^ 0x80) < 0x40 && ((uchar)start[3] ^ 0x80) < 0x40 &&
            (c >= 0xf1 || (uchar)start[1] >= 0x90) && (c <= 0xf3 || (uchar)start[1] <= 0x8F))) {
         return 0; /* invalid utf8 character */
      }
      return 4;
   }
...
static unsigned int check_mb_big5(const char *start, const char *end) {
   return (valid_big5head(*((const uchar *)start)) && (end - start) > 1 && valid_big5tail(*((const uchar *)start + 1))
               ? 2
               : 0);
}

static unsigned int mysql_mbcharlen_big5(unsigned int big5) { return (valid_big5head(big5)) ? 2 : 1; }
/* }}} */
...
static unsigned int mysql_mbcharlen_cp932(unsigned int cp932) { return (valid_cp932head((uchar)cp932)) ? 2 : 1; }
/* }}} */
...
static unsigned int mysql_mbcharlen_euckr(unsigned int kr) { return (valid_euckr(kr)) ? 2 : 1; }
/* }}} */
...
static unsigned int check_mb_eucjpms(const char *start, const char *end) {
   ...
   if (valid_eucjpms_ss3(start[0]) && (end - start) > 2 && valid_eucjpms(start[1]) && valid_eucjpms(start[2])) {
      return 2;
   }
   return 0;
}
...
static unsigned int check_mb_gb2312(const char *start, const char *end) {
   return (valid_gb2312_head((unsigned int)start[0]) && end - start > 1 && valid_gb2312_tail((unsigned int)start[1]))
              ? 2
              : 0;
}

static unsigned int mysql_mbcharlen_gb2312(unsigned int gb) { return (valid_gb2312_head(gb)) ? 2 : 1; }
/* }}} */
...
static unsigned int mysql_mbcharlen_gbk(unsigned int gbk) { return (valid_gbk_head(gbk) ? 2 : 1); }
/* }}} */
...
static unsigned int mysql_mbcharlen_sjis(unsigned int sjis) { return (valid_sjis_head((uchar)sjis)) ? 2 : 1; }
/* }}} */
...
static unsigned int mysql_mbcharlen_ucs2(unsigned int ucs2 __attribute((unused))) { return 2; /* always 2 */ }
/* }}} */
...
static uint mysql_mbcharlen_utf16(unsigned int utf16) { return UTF16_HIGH_HEAD(utf16) ? 4 : 2; }
/* }}} */

/* {{{ utf32 functions */
static uint check_mb_utf32(const char *start __attribute((unused)), const char *end __attribute((unused))) { return 4; }

static uint mysql_mbcharlen_utf32(unsigned int utf32 __attribute((unused))) { return 4; }
/* }}} */
...
static unsigned int check_mb_gb18030_valid(const char *start, const char *end) {
   ...
   if (is_gb18030_even_2(start[1])) {
      return 2;
   } else if (end - start > 3 && is_gb18030_even_4(start[1]) && is_gb18030_odd(start[2]) &&
              is_gb18030_even_4(start[3])) {
      return 4;
   }
...
const MARIADB_CHARSET_INFO mariadb_compiled_charsets[] = {
    ...
    {33, 1, UTF8_MB3, UTF8_MB3 "_general_ci", "", 65001, "UTF-8", 1, 3, mysql_mbcharlen_utf8mb3,
    ...
    {76, 1, UTF8_MB3, UTF8_MB3 "_tolower_ci", "", 65001, "UTF-8", 1, 3, mysql_mbcharlen_utf8mb3,
     check_mb_utf8mb3_valid},
    ...
    {159, 1, "ucs2", "ucs2_general_mysql500_ci", "", 1200, "UCS2-BE", 2, 2, mysql_mbcharlen_ucs2,
     check_mb_ucs2}, /* MDB */
    ...
    {192, 1, UTF8_MB3, UTF8_MB3 "_general_ci", "", 65001, "UTF-8", 1, 3, mysql_mbcharlen_utf8mb3,
     check_mb_utf8mb3_valid},
    {193, 1, UTF8_MB3, UTF8_MB3 "_icelandic_ci", "", 65001, "UTF-8", 1, 3, mysql_mbcharlen_utf8mb3,
     check_mb_utf8mb3_valid},
    {194, 1, UTF8_MB3, UTF8_MB3 "_latvian_ci", "", 65001, "UTF-8", 1, 3, mysql_mbcharlen_utf8mb3,
     check_mb_utf8mb3_valid},
    {195, 1, UTF8_MB3, UTF8_MB3 "_romanian_ci", "", 65001, "UTF-8", 1, 3, mysql_mbcharlen_utf8mb3,
     check_mb_utf8mb3_valid},
    {196, 1, UTF8_MB3, UTF8_MB3 "_slovenian_ci", "", 65001, "UTF-8", 1, 3, mysql_mbcharlen_utf8mb3,
     check_mb_utf8mb3_valid},
    {197, 1, UTF8_MB3, UTF8_MB3 "_polish_ci", "", 65001, "UTF-8", 1, 3, mysql_mbcharlen_utf8mb3,
     check_mb_utf8mb3_valid},
    {198, 1, UTF8_MB3, UTF8_MB3 "_estonian_ci", "", 65001, "UTF-8", 1, 3, mysql_mbcharlen_utf8mb3,
     check_mb_utf8mb3_valid},
    {199, 1, UTF8_MB3, UTF8_MB3 "_spanish_ci", "", 65001, "UTF-8", 1, 3, mysql_mbcharlen_utf8mb3,
     check_mb_utf8mb3_valid},
    {119, 1, UTF8_MB3, UTF8_MB3 "_spanish_ci", "", 65001, "UTF-8", 1, 3, mysql_mbcharlen_utf8mb3,
     check_mb_utf8mb3_valid},
    {200, 1, UTF8_MB3, UTF8_MB3 "_swedish_ci", "", 65001, "UTF-8", 1, 3, mysql_mbcharlen_utf8mb3,
     check_mb_utf8mb3_valid},
    {201, 1, UTF8_MB3, UTF8_MB3 "_turkish_ci", "", 65001, "UTF-8", 1, 3, mysql_mbcharlen_utf8mb3,
     check_mb_utf8mb3_valid},
    {202, 1, UTF8_MB3, UTF8_MB3 "_czech_ci", "", 65001, "UTF-8", 1, 3, mysql_mbcharlen_utf8mb3, check_mb_utf8mb3_valid},
    {203, 1, UTF8_MB3, UTF8_MB3 "_danish_ci", "", 65001, "UTF-8", 1, 3, mysql_mbcharlen_utf8mb3,
     check_mb_utf8mb3_valid},
    {204, 1, UTF8_MB3, UTF8_MB3 "_lithuanian_ci", "", 65001, "UTF-8", 1, 3, mysql_mbcharlen_utf8mb3,
     check_mb_utf8mb3_valid},
    {205, 1, UTF8_MB3, UTF8_MB3 "_slovak_ci", "", 65001, "UTF-8", 1, 3, mysql_mbcharlen_utf8mb3,
     check_mb_utf8mb3_valid},
    {206, 1, UTF8_MB3, UTF8_MB3 "_spanish2_ci", "", 65001, "UTF-8", 1, 3, mysql_mbcharlen_utf8mb3,
     check_mb_utf8mb3_valid},
    {207, 1, UTF8_MB3, UTF8_MB3 "_roman_ci", "", 65001, "UTF-8", 1, 3, mysql_mbcharlen_utf8mb3, check_mb_utf8mb3_valid},
    {208, 1, UTF8_MB3, UTF8_MB3 "_persian_ci", "", 65001, "UTF-8", 1, 3, mysql_mbcharlen_utf8mb3,
     check_mb_utf8mb3_valid},
    {209, 1, UTF8_MB3, UTF8_MB3 "_esperanto_ci", "", 65001, "UTF-8", 1, 3, mysql_mbcharlen_utf8mb3,
     check_mb_utf8mb3_valid},
    {210, 1, UTF8_MB3, UTF8_MB3 "_hungarian_ci", "", 65001, "UTF-8", 1, 3, mysql_mbcharlen_utf8mb3,
     check_mb_utf8mb3_valid},
    {211, 1, UTF8_MB3, UTF8_MB3 "_sinhala_ci", "", 65001, "UTF-8", 1, 3, mysql_mbcharlen_utf8mb3,
     check_mb_utf8mb3_valid},
    {212, 1, UTF8_MB3, UTF8_MB3 "_german2_ci", "", 65001, "UTF-8", 1, 3, mysql_mbcharlen_utf8mb3,
     check_mb_utf8mb3_valid},
    {214, 1, UTF8_MB3, UTF8_MB3 "_unicode_520_ci", "", 65001, "UTF-8", 1, 3, mysql_mbcharlen_utf8mb3,
     check_mb_utf8mb3_valid},
    {215, 1, UTF8_MB3, UTF8_MB3 "_vietnamese_ci", "", 65001, "UTF-8", 1, 3, mysql_mbcharlen_utf8mb3,
     check_mb_utf8mb3_valid},
    {213, 1, UTF8_MB3, UTF8_MB3 "_croatian_ci", "", 65001, "UTF-8", 1, 3, mysql_mbcharlen_utf8mb3,
     check_mb_utf8mb3_valid}, /*MDB*/
    {223, 1, UTF8_MB3, UTF8_MB3 "_general_mysql500_ci", "", 65001, "UTF-8", 1, 3, mysql_mbcharlen_utf8mb3,
     check_mb_utf8mb3_valid}, /*MDB*/
    ...
    {245, 1, UTF8_MB4, UTF8_MB4 "_croatian_mysql561_ci", "", 65001, "UTF-8", 1, 4, mysql_mbcharlen_utf8,
     check_mb_utf8_valid},
    ...
    {248, 1, "gb18030", "gb18030_chinese_ci", "", 54936, "GB18030", 1, 4, mysql_mbcharlen_gb18030,
     check_mb_gb18030_valid},
    {249, 1, "gb18030", "gb18030_bin", "", 54936, "GB18030", 1, 4, mysql_mbcharlen_gb18030, check_mb_gb18030_valid},
    {250, 1, "gb18030", "gb18030_unicode_520_ci", "", 54936, "GB18030", 1, 4, mysql_mbcharlen_gb18030,
     check_mb_gb18030_valid},
    ...
    {256, 1, UTF8_MB4, UTF8_MB4 "_de_pb_0900_ai_ci", "", 65001, "UTF-8", 1, 4, mysql_mbcharlen_utf8,
     check_mb_utf8_valid},
    ...
    {270, 1, UTF8_MB4, UTF8_MB4 "_es_trad_0900_ai_ci", "", 65001, "UTF-8", 1, 4, mysql_mbcharlen_utf8,
     check_mb_utf8_valid},
    ...
    {279, 1, UTF8_MB4, UTF8_MB4 "_de_pb__0900_as_cs", "", 65001, "UTF-8", 1, 4, mysql_mbcharlen_utf8,
     check_mb_utf8_valid},
    ...
    {293, 1, UTF8_MB4, UTF8_MB4 "_es_trad_0900_as_cs", "", 65001, "UTF-8", 1, 4, mysql_mbcharlen_utf8,
     check_mb_utf8_valid},
    ...
    {304, 1, UTF8_MB4, UTF8_MB4 "_ja_0900_as_cs_ks", "", 65001, "UTF-8", 1, 4, mysql_mbcharlen_utf8,
     check_mb_utf8_valid},
    ...
    {576, 1, UTF8_MB3, UTF8_MB3 "_croatian_ci", "", 65001, "UTF-8", 1, 3, mysql_mbcharlen_utf8mb3,
     check_mb_utf8mb3_valid}, /*MDB*/
    {577, 1, UTF8_MB3, UTF8_MB3 "_myanmar_ci", "", 65001, "UTF-8", 1, 3, mysql_mbcharlen_utf8mb3,
     check_mb_utf8mb3_valid}, /*MDB*/
    {578, 1, UTF8_MB3, UTF8_MB3 "_thai_520_w2", "", 65001, "UTF-8", 1, 3, mysql_mbcharlen_utf8mb3,
     check_mb_utf8mb3_valid}, /*MDB*/
    {579, 1, UTF8_MB3, UTF8_MB3 "_general1400_as_ci", "", 65001, "UTF-8", 1, 3, mysql_mbcharlen_utf8mb3,
     check_mb_utf8mb3_valid}, /*MDB*/
    ...
    {611, 1, UTF8_MB4, UTF8_MB4 "_general1400_as_ci", "", 65001, "UTF-8", 1, 3, mysql_mbcharlen_utf8,
     check_mb_utf8_valid},
    ...
    {1057, 1, UTF8_MB3, UTF8_MB3 "_general_nopad_ci", "", 65001, "UTF-8", 1, 3, mysql_mbcharlen_utf8mb3,
     check_mb_utf8mb3_valid},
    ...
    {1069, 1, UTF8_MB4, UTF8_MB4 "_general_nopad_ci", "", 65001, "UTF-8", 1, 4, mysql_mbcharlen_utf8,
     check_mb_utf8_valid},
    {1070, 1, UTF8_MB4, UTF8_MB4 "_general_nopad_bin", "", 65001, "UTF-8", 1, 4, mysql_mbcharlen_utf8,
     check_mb_utf8_valid},
    ...
    {1107, 1, UTF8_MB3, UTF8_MB3 "_nopad_bin", "", 65001, "UTF-8", 1, 3, mysql_mbcharlen_utf8mb3,
     check_mb_utf8mb3_valid},
    ...
    {1121, 1, "eucjpms", "eucjpms_japanese_nopad_ci", "", 932, "EUCJP-MS", 1, 3, mysql_mbcharlen_eucjpms,
     check_mb_eucjpms},
    ...
    {1216, 1, UTF8_MB3, UTF8_MB3 "_unicode_nopad_ci", "", 65001, "UTF-8", 1, 3, mysql_mbcharlen_utf8mb3,
     check_mb_utf8mb3_valid},
    {1238, 1, UTF8_MB3, UTF8_MB3 "_unicode_520_nopad_ci", "", 65001, "UTF-8", 1, 3, mysql_mbcharlen_utf8mb3,
     check_mb_utf8mb3_valid},
    {1248, 1, UTF8_MB4, UTF8_MB4 "_unicode_nopad_ci", "", 65001, "UTF-8", 1, 4, mysql_mbcharlen_utf8,
     check_mb_utf8_valid},
    {1270, 1, UTF8_MB4, UTF8_MB4 "_unicode_520_nopad_ci", "", 65001, "UTF-8", 1, 4, mysql_mbcharlen_utf8,
     check_mb_utf8_valid},
...
*/
static MY_UCA1400_COLLATION_DEFINITION my_uca1400_collation_definitions[MY_UCA1400_COLLATION_DEFINITION_COUNT] = {
#define COLDEF(tl, name, id_utf8mb3, id_utf8mb4, id_ucs2, id_utf16, id_utf32) \
   {(""), (name), (id_utf8mb3), (id_utf8mb4), (id_ucs2), (id_utf16), (id_utf32)}
    /* Block N1 */
    COLDEF("", "", 192, 224, 128, 101, 160), COLDEF(icelandic, "icelandic", 193, 225, 129, 102, 161),
    COLDEF(latvian, "latvian", 194, 226, 130, 103, 162), COLDEF(romanian, "romanian", 195, 227, 131, 104, 163),
    COLDEF(slovenian, "slovenian", 196, 228, 132, 105, 164), COLDEF(polish, "polish", 197, 229, 133, 106, 165),
    COLDEF(estonian, "estonian", 198, 230, 134, 107, 166), COLDEF(spanish, "spanish", 199, 231, 135, 108, 167),
    COLDEF(swedish, "swedish", 200, 232, 136, 109, 168), COLDEF(turkish, "turkish", 201, 233, 137, 110, 169),
    COLDEF(czech, "czech", 202, 234, 138, 111, 170), COLDEF(danish, "danish", 203, 235, 139, 112, 171),
    COLDEF(lithuanian, "lithuanian", 204, 236, 140, 113, 172), COLDEF(slovak, "slovak", 205, 237, 141, 114, 173),
    COLDEF(spanish2, "spanish2", 206, 238, 142, 115, 174), COLDEF(roman, "roman", 207, 239, 143, 116, 175),
    COLDEF(persian, "persian", 208, 240, 144, 117, 176), COLDEF(esperanto, "esperanto", 209, 241, 145, 118, 177),
    COLDEF(hungarian, "hungarian", 210, 242, 146, 119, 178), COLDEF(sinhala, "sinhala", 211, 243, 147, 120, 179),
...
static inline my_bool my_collation_id_is_uca1400(uint id) {
   return (my_bool)(id >= MY_UCA1400_COLLATION_ID_POSSIBLE_MIN && id <= MY_UCA1400_COLLATION_ID_POSSIBLE_MAX);
}

static inline uint my_uca1400_collation_id_to_tailoring_id(uint id) { return (id >> 3) & 0x1F; }

static inline my_cs_encoding_t my_uca1400_collation_id_to_charset_id(uint id) {
...
static uint my_uca1400_collation_id_uca400_compat(uint id) {
   ...
   if (!(def = &my_uca1400_collation_definitions[tlid])->name) return id;
   switch (csid) {
      case MY_CS_ENCODING_UTF8MB3:
         return def->id_utf8mb3;
      case MY_CS_ENCODING_UTF8MB4:
         return def->id_utf8mb4;
...
const MARIADB_CHARSET_INFO *mysql_find_charset_nr(unsigned int charsetnr) {
   ...
   if (my_collation_id_is_uca1400(charsetnr)) charsetnr = my_uca1400_collation_id_uca400_compat(charsetnr);
...
MARIADB_CHARSET_INFO *mysql_find_charset_name(const char *name) {
   ...
   if (!strcasecmp("utf8", csname)) csname = "utf8mb3";
...
size_t mysql_cset_escape_quotes(const MARIADB_CHARSET_INFO *cset, char *newstr, const char *escapestr,
...
size_t mysql_cset_escape_slashes(const MARIADB_CHARSET_INFO *cset, char *newstr, const char *escapestr,
                                 size_t escapestr_len) {
   const char *newstr_s = newstr;
   const char *newstr_e = newstr + 2 * escapestr_len;
   const char *end = escapestr + escapestr_len;
...
#endif
   if (!p) return MADB_DEFAULT_CHARSET_NAME;
   while (MADB_OS_CHARSET[i].identifier) {
      if (MADB_OS_CHARSET[i].supported > MADB_CS_UNSUPPORTED && strcasecmp(MADB_OS_CHARSET[i].identifier, p) == 0)
...
int madb_get_windows_cp(const char *charset) {
   unsigned int i = 0;
   while (MADB_OS_CHARSET[i].identifier) {
      if (MADB_OS_CHARSET[i].supported > MADB_CS_UNSUPPORTED && strcmp(MADB_OS_CHARSET[i].charset, charset) == 0)
...
size_t STDCALL mariadb_convert_string(const char *from __attribute__((unused)),
                                      ...
                                      char *to __attribute__((unused)), size_t *to_len __attribute__((unused)),
   ...
   *errorcode = 0;
   ...
   if (!from_cs || !from_cs->encoding || !from_cs->encoding[0] || !to_cs || !to_cs->encoding || !to_cs->encoding[0]) {
      *errorcode = EINVAL;
      return rc;
   }
