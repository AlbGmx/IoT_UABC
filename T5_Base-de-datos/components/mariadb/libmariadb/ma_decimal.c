...
int bin2decimal(const char *from, decimal *to, int precision, int scale) {
   int error = E_DEC_OK, intg = precision - scale, intg0 = intg / DIG_PER_DEC1, frac0 = scale / DIG_PER_DEC1,
       ...
       frac1 = frac0 + (frac0x > 0), tmp_size = decimal_bin_size(precision, scale);
   char *tmp;
   ...
   *tmp ^= 0x80; /* remove sign bit */
   ...
   if (intg0x) {
      ...
      switch (i) {
         case 1:
            x = myisam_sint1korr(from);
            break;
         case 2:
            x = myisam_sint2korr(from);
            break;
         case 3:
            x = myisam_sint3korr(from);
            break;
         case 4:
            x = myisam_sint4korr(from);
            break;
         default:
            DBUG_ASSERT(0);
            x = 0;
