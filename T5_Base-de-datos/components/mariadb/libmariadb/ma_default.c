...
my_bool _mariadb_read_options(MYSQL *mysql, const char *config_dir, const char *config_file, const char *group,
...
void release_configuration_dirs() {
   if (configuration_dirs) {
      int i = 0;
      while (configuration_dirs[i]) free(configuration_dirs[i++]);
...
char **get_default_configuration_dirs() {
   ...
   if (!configuration_dirs) goto end;
   ...
   */
   ...
   if (!GetSystemWindowsDirectory(dirname, FN_REFLEN) || add_cfg_dir(configuration_dirs, dirname)) goto error;

   if (!GetWindowsDirectory(dirname, FN_REFLEN) || add_cfg_dir(configuration_dirs, dirname)) goto error;

   if (add_cfg_dir(configuration_dirs, "C:")) goto error;

   if (GetModuleFileName(NULL, dirname, FN_REFLEN)) {
      PathRemoveFileSpec(dirname);
      if (add_cfg_dir(configuration_dirs, dirname)) goto error;
   }
#else
   if (!(env = getenv("MARIADB_HOME"))) env = getenv("MYSQL_HOME");
   if (env && add_cfg_dir(configuration_dirs, env)) goto error;
end:
...
static my_bool is_group(char *ptr, const char **groups) {
   while (*groups) {
      if (!strcmp(ptr, *groups)) return 1;
      groups++;
   }
   return 0;
}

static my_bool _mariadb_read_options_from_file(MYSQL *mysql, const char *config_file, const char *group,
                                               unsigned int recursion) {
   ...
   if (!(file = ma_open(config_file, "r", NULL))) goto err;
      ...
      *end = 0;
      if (value) {
         ...
         *value_end = 0;
         ...
         if (*value == '\'' || *value == '\"') {
            value++;
            if (value_end[-1] == '\'' || value_end[-1] == '\"') value_end--;
         }
...
err:
   if (file) ma_close(file);
   return rc;
}

my_bool _mariadb_read_options(MYSQL *mysql, const char *config_dir, const char *config_file, const char *group,
                              unsigned int recursion) {
   int i = 0, exts, errors = 0;
   char filename[FN_REFLEN + 1];
   unsigned int recursion_stop = 64;

   if (recursion >= recursion_stop) return 1;

   if (config_file && config_file[0]) return _mariadb_read_options_from_file(mysql, config_file, group, recursion);

   if (config_dir && config_dir[0]) {
      for (exts = 0; ini_exts[exts]; exts++) {
         snprintf(filename, FN_REFLEN, "%s%cmy.%s", config_dir, FN_LIBCHAR, ini_exts[exts]);
         if (!access(filename, R_OK)) errors += _mariadb_read_options_from_file(mysql, filename, group, recursion);
      }
      return errors;
   }
   ...
   for (i = 0; i < MAX_CONFIG_DIRS && configuration_dirs[i]; i++) {
      for (exts = 0; ini_exts[exts]; exts++) {
         snprintf(filename, FN_REFLEN, "%s%cmy.%s", configuration_dirs[i], FN_LIBCHAR, ini_exts[exts]);
         if (!access(filename, R_OK)) errors += _mariadb_read_options_from_file(mysql, filename, group, recursion);
      }
   }
   return errors;
